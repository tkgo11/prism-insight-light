...
export function JeoninguLabPage({ data }: JeoninguLabPageProps) {
  ...
  // 데이터가 없는 경우
  const hasNoData = !data.enabled || !timeline || timeline.length === 0
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <CardContent>
            <div className="flex items-center justify-between">
              <div className={`text-2xl font-bold ${summary?.cumulative_return >= 0 ? "text-green-600" : "text-red-600"}`}>
                ...
                {summary?.cumulative_return?.toFixed(2) || "0.00"}%
              </div>
        ...
        <Card className="border-green-200 bg-gradient-to-br from-green-50 to-white">
          ...
          <CardContent>
            <div className="flex items-center justify-between">
              <div className="text-2xl font-bold text-green-600">{summary?.win_rate?.toFixed(1) || "0.0"}%</div>
              <div className="text-3xl">🎯</div>
            </div>
          ...
          <CardContent>
            <div className="flex items-center justify-between">
              <div className="text-2xl font-bold text-yellow-600">
                {summary ? `${(summary.current_balance / 10000).toFixed(0)}만원` : "1,000만원"}
              </div>
              <div className="text-3xl">💰</div>
            </div>
    ...
    <div className="space-y-6">
        ...
        <div className="relative z-10 space-y-4">
          ...
          <div className="mt-4 p-3 bg-yellow-500/20 rounded-lg border border-yellow-300/30">
            <p className="text-xs text-yellow-50">
              <strong>⚠️ 면책 조항:</strong> 본 페이지의 정보는 투자 권유가 아닌 참고용 정보입니다. 
              모든 투자 결정과 그 결과에 대한 책임은 투자자 본인에게 있으며, 
              본 실험 결과를 근거로 한 실제 투자 손실에 대해 어떠한 책임도 지지 않습니다.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className="border-blue-200 bg-gradient-to-br from-blue-50 to-white">
          ...
          <CardContent>
            <div className="flex items-center justify-between">
              <div className="text-2xl font-bold text-blue-600">{summary.total_trades}건</div>
              <div className="text-3xl">💼</div>
            </div>
            <div className="text-xs text-muted-foreground mt-1">
              평균 {summary.avg_return_per_trade >= 0 ? "+" : ""}
        ...
        <Card className="border-yellow-200 bg-gradient-to-br from-yellow-50 to-white">
          ...
          <CardContent>
            <div className="flex items-center justify-between">
              <div className="text-2xl font-bold text-yellow-600">
                {(summary.current_balance / 10000).toFixed(0)}만원
              </div>
              <div className="text-3xl">💰</div>
            </div>
            <div className="text-xs text-muted-foreground mt-1">초기 1천만원</div>
