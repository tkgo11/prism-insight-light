...
ensure_supported_host() {
    ...
    if [ "$INSTALL_ACTION" = "install" ] && [ -z "$REPO_DIR" ]; then
        require_command tar
...
ensure_project_checkout() {
    ...
    (umask 077; printf '%s\n' "$SCRIPT_VERSION" > "$INSTALL_DIR/.prism-insight-install")
    PROJECT_DIR="$INSTALL_DIR"
...
ensure_existing_project_checkout() {
    ...
    log_warn "체크아웃 없이 제한된 복구 제거를 시도합니다."
    return 1
}
...
default_unit_asset_percent: 100
default_unit_asset_percent_usd: 100
auto_exchange_usd_on_buy: true
auto_exchange_buffer_percent: 5.0
max_auto_exchange_krw: null
auto_exchange_min_shortfall_usd: 0.0
auto_trading: true
default_mode: $(yaml_quote "$kis_default_mode")
default_product_code: $(yaml_quote "$kis_account_product")
my_app: $(yaml_quote "$kis_my_app")
my_sec: $(yaml_quote "$kis_my_sec")
paper_app: $(yaml_quote "$kis_paper_app")
paper_sec: $(yaml_quote "$kis_paper_sec")
my_htsid: $(yaml_quote "$kis_my_htsid")
accounts:
  - name: $(yaml_quote "$kis_account_name")
    mode: $(yaml_quote "$kis_account_mode")
    market: $(yaml_quote "$kis_account_market")
    account: $(yaml_quote "$kis_account_number")
    product: $(yaml_quote "$kis_account_product")
signal_strategy:
  name: "balanced_risk"
  split_count: 1
  ...
  full_exit_reasons:
    ...
    - risk_off
    - manual_exit
  buy_buffer_percent: 5.0
  sell_buffer_percent: 5.0
  us_price_decimals: 2
  kr_tick_rounding: 1
  window_minutes: 1
...
my_token: ""
my_agent: $(yaml_quote "$kis_my_agent")
EOF

    chmod 600 "$KIS_CONFIG_PATH"
...
strip_recovery_cron_block() {
    local input_file="$1"
    local output_file="$2"
    local begin_marker="# BEGIN PRISM-INSIGHT SUBSCRIBER DOCKER CRON"
    local end_marker="# END PRISM-INSIGHT SUBSCRIBER DOCKER CRON"
...
recovery_uninstall_crontab() {
    local current_file
    local clean_file

    if ! command_exists "$CRONTAB_BIN"; then
    ...
    current_file="$(mktemp)"
    clean_file="$(mktemp)"
    "$CRONTAB_BIN" -l > "$current_file"
    if ! strip_recovery_cron_block "$current_file" "$clean_file"; then
        rm -f "$current_file" "$clean_file"
        log_error "관리 마커가 손상되어 기존 crontab을 변경하지 않습니다."
        return 1
    fi
    if [ -s "$clean_file" ]; then
        "$CRONTAB_BIN" "$clean_file"
    else
        "$CRONTAB_BIN" -r
    fi
    rm -f "$current_file" "$clean_file"
    log_success "손상된 체크아웃의 subscriber Docker crontab을 제거했습니다."
}
...
recovery_remove_docker_artifacts() {
    local status=0
    if ! command_exists "$DOCKER_BIN"; then
        log_error "Docker 실행 파일을 찾을 수 없어 컨테이너와 이미지를 확인할 수 없습니다: $DOCKER_BIN"
        return 1
    fi
    if recovery_container_exists; then
        if recovery_container_running; then
...
recover_without_checkout() {
    local action="$1"
    PROJECT_DIR="${REPO_DIR:-$INSTALL_DIR}"
    recovery_uninstall_crontab
...
remove_install_remnant_if_recognizable() {
    if [ -n "$REPO_DIR" ] || [ ! -d "$PROJECT_DIR" ]; then
        return 0
    fi
    if [ -f "$PROJECT_DIR/.prism-insight-install" ] || {
        [ -f "$PROJECT_DIR/Dockerfile" ] &&
        [ -f "$PROJECT_DIR/subscriber.py" ] &&
        [ -f "$PROJECT_DIR/.env.example" ] &&
        [ -f "$PROJECT_DIR/trading/config/kis_devlp.yaml.example" ]
    }; then
        remove_install_dir
        return 0
    fi
    log_warn "프로젝트 흔적을 확인할 수 없어 디렉터리는 보존합니다: $PROJECT_DIR"
}
...
예시:
  curl -fsSLO https://raw.githubusercontent.com/$DEFAULT_REPO_OWNER/$DEFAULT_REPO_NAME/main/install_prism_docker.sh
...
main() {
    ...
    if [ "$INSTALL_ACTION" = "uninstall" ]; then
        ...
        if ensure_existing_project_checkout; then
            ensure_repo_paths
            run_repo_setup uninstall
        else
            recover_without_checkout uninstall
        fi
        if [ -n "$REPO_DIR" ]; then
    ...
    if [ "$INSTALL_ACTION" = "uninstall-cron" ]; then
        ...
        if ensure_existing_project_checkout; then
            ...
            recover_without_checkout uninstall-cron
        fi
        print_cron_uninstall_summary
        exit 0
    fi
