...
# Fallback to local config if project root config doesn't exist
...
class USStockTrading:
    ...
    DEFAULT_BUY_AMOUNT = _cfg.get("default_unit_amount_usd", 100) if _cfg else 100 # $100 default
    # Auto trading enabled flag
    AUTO_TRADING = _cfg.get("auto_trading", True) if _cfg else True
    # Default trading environment
    DEFAULT_MODE = _cfg.get("default_mode", "demo") if _cfg else "demo"
...
class AsyncUSTradingContext:
    ...
    DEFAULT_BUY_AMOUNT = _cfg.get("default_unit_amount_usd", 100) if _cfg else 100
    AUTO_TRADING = _cfg.get("auto_trading", True) if _cfg else True
    DEFAULT_MODE = _cfg.get("default_mode", "demo") if _cfg else "demo"
...
if __name__ == "__main__":
    ...
    try:
        ...
        # exit(1) # Don't exit in case of import error just for test
