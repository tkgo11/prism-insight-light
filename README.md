...
│ Trading Flow Analyst   → 투자자 동향         │
│ Financial Analyst      → 재무 분석          │
│ Industry Analyst       → 사업 분석          │
│ Information Analyst    → 뉴스 분석          │
...
│   💬 커뮤니케이션 팀               │   │ 💬 사용자 상담팀      │
├────────────────────────────────┤   ├────────────────────┤
│ Summary Specialist ⇄ Quality   │   │ Portfolio          │
...
**자동화 파이프라인** (메인 플로우):
- 급등주 포착부터 매매 시뮬레이션까지 완전 자동화
- 6개 분석 에이전트가 병렬로 작동하여 빠른 분석
- 전략 통합 → 품질 검증 → 매매 의사결정의 순차 처리

**사용자 직접 인터랙션** (별도 플로우):
- 텔레그램 봇을 통한 개인 맞춤형 종목 평가
- 대화형 AI로 추가 질문 처리 및 맥락 유지
