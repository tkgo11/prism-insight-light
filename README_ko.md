...
과거 README 에 등장하는 `cores/`, `utils/`, `examples/streamlit/` 등의 디렉터리는 **이 스냅샷에는 존재하지 않으므로 무시하셔도 됩니다.**
...
Pub/Sub 구독자를 사용하려면 `.env`에 최소한 다음 값을 채워 넣어야 합니다.
...
KIS 매매 관련 설정은 `trading/config/kis_devlp.yaml.example`를 복사/수정하여 사용합니다.
...
1. `.env`에 GCP 및 (선택) KIS 관련 설정을 완료합니다.
2. 드라이런(실제 매매 X) 모드 실행:

```bash
python gcp_pubsub_subscriber.py --dry-run
```

3. 라이브(실매매 가능, 주의) 모드 실행:
...
## EXTERNAL_SUBSCRIBER_GUIDE 내용 통합 (전체)

아래는 원래 `EXTERNAL_SUBSCRIBER_GUIDE.md` 파일에 있던 **실시간 트레이딩 시그널 구독 가이드 전체 내용**입니다.
