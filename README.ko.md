...
기존 설치는 **기본적으로 단일 계좌 동작을 유지**합니다. 검증된 자동 신호를 같은 모드·시장에 맞는 활성 계좌마다 독립적으로 실행하려면 아래 설정을 명시적으로 켜세요.
...
분배기는 비활성 계좌, 다른 모드 계좌, 시장이 맞지 않는 계좌를 주문 전에 제외합니다. 적격 계좌는 각자의 KIS 환경·자격 증명·잔고/보유 종목·위험 제한·매수 금액/비율을 사용해 기존 전략을 독립 실행합니다. KIS 환경 활성화가 공유 상태이므로 브로커 작업은 의도적으로 직렬화되며, 한 계좌의 인증 또는 주문 실패는 계좌별 결과로 기록될 뿐 다른 적격 계좌의 실행을 중단하지 않습니다.

장외 대기열은 원래의 대상 계좌를 식별 가능한 비밀 정보 없이 보존하고 해당 대상에만 재생합니다. 자동 신호/계좌 조합은 7일 보존 실행 원장으로 관리되어 Pub/Sub 재전달, 재시작, 재시도, 네트워크 불확실성으로 인한 중복 주문을 차단합니다. 대기 중인 계좌가 이후 비활성화·삭제·시장 비호환 상태가 되면 결정적으로 건너뜁니다. 드라이런도 동일한 계좌 선택을 적용해 계좌별 예상 결과를 반환하며 실제 주문은 전송하지 않습니다.
...
- subscriber, 별도 WebUI/cron 등 여러 프로세스의 broker workflow는 공용 파일 락으로 직렬화됩니다.
- stop-loss watcher가 실제 보유수량을 확인하지 못하면 보유 중이라고 추정하지 않고 SELL을 차단합니다. stop-loss 원장이 손상되어도 빈 원장으로 간주하지 않습니다.
- WebUI 수동 주문은 사용자가 의도적으로 반복할 수 있으므로 이 7일 자동 신호 dedupe에서는 제외됩니다. 대신 기존 CSRF, 일회용 주문 티켓, 확인 문구 보호를 그대로 사용합니다.

`unknown` 상태가 발생하면 KIS 주문/체결 내역을 확인한 뒤 운영자가 조치하세요. 자동 retry는 안전상 수행하지 않습니다.
...
- **화면:** 반응형 탐색 메뉴, 다크/라이트/시스템 테마, 화면 밀도 조절, `Ctrl+K` 또는 `Cmd+K` 페이지 검색, 서울·뉴욕 시계.
- **대기열:** 종목·회사·날짜 검색, 시장/상태/동작 필터, 시간대 기준 날짜 정렬, 페이지 크기 선택, 실행 예정 시각 안내, 실패 사유와 필터 결과 CSV 저장.
- **실행 기록:** `/activity`에서 자동 실행 중복 방지 원장의 상태별 집계와 최근 최대 500건을 읽기 전용으로 확인합니다. 체결 내역이나 전체 주문 이력이 아니며, 계좌·종목 메타데이터를 추정하지 않습니다.
- **로그:** 소스 선택, 등급 및 문자열 검색, 최대 500줄/64 KB 제한, 마스킹된 결과 다운로드·복사, 줄바꿈 전환.
- **시그널:** KR/US 예제, JSON 정리·복사·초기화, 검증된 시그널을 그대로 안전한 시뮬레이터로 전달. 예제 가격은 설명용입니다.
- **조회 편의:** 계좌 목록 검색, 조회 화면의 수동 새로고침과 선택적 30/60초 자동 갱신. 숨겨진 탭과 입력/필터 편집 중에는 자동 갱신을 중단합니다.

테마와 화면 밀도만 로컬 저장소에 남습니다. 자동 갱신용 세션 저장소에는 경로/주기만 저장하며 시그널 본문·계좌·로그는 브라우저 저장소에 저장하지 않습니다. JavaScript를 끄면 기존 서버 렌더링과 폼 제출을 사용할 수 있습니다. 필터·CSV는 화면에 로드된 제한 범위에만 적용됩니다. 설정 준비 상태는 subscriber 생존 확인이나 브로커 연결 확인을 의미하지 않습니다.
